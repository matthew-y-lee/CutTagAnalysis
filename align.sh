#!/bin/bash
echo "Align.sh running"
cores=8

genome="e_coli"

ref="/Users/matthewlee/Desktop/Columbia/Sequencing/Drosophila_melanogaster_UCSC_dm6/Drosophila_melanogaster/UCSC/dm6/dm6"
projPath="/Users/matthewlee/Desktop/Columbia/Sequencing"

Ecoli_ref="/Users/matthewlee/Desktop/Columbia/Sequencing/Escherichia_coli_K_12_MG1655_NCBI_2001-10-15/Escherichia_coli_K_12_MG1655/NCBI/2001-10-15/Sequence/Bowtie2Index/genome"
...
if [[ "${genome}" == "fly" ]]; then
	echo "Fly genome alignment selected"
	for i in "${!input1[@]}"; do
		bowtie2 --end-to-end --very-sensitive --no-mixed --no-discordant --phred33 -I 10 -X 700 -p ${cores}  -x ${ref} -1 ${input1[i]} -2 ${input2[i]} -S ${projPath}/alignment/sam/${proteinName[i]}_bowtie2.sam &> ${projPath}/alignment/sam/bowtie2_summary/${proteinName[i]}_bowtie2.txt
	done
elif [[ "${genome}" == "e_coli" ]]; then 
	echo "E coli alignment selected"
	for i in "${!input1[@]}"; do
        bowtie2 --end-to-end --very-sensitive --no-mixed --no-discordant --phred33 -I 10 -X 700 -p ${cores}  -x ${Ecoli_ref} -1 ${input1[i]} -2 ${input2[i]} -S /dev/null &> ${projPath}/alignment/sam/bowtie2_summary/${proteinName[i]}_E_coli_bowtie2.txt
	done
else 
	echo "no genome selected"
fi
